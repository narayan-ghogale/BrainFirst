...
if(isset($_FILES['video_upload']))
{
    ...
    $ModifiedVideoName = preg_replace('/\s+/', '_',$fileName);
    $targetName="content/".$course_id."_".$ModifiedVideoName;
    $q = "select * from course_content where (course_id='$course_id' AND file_type='Video' AND file_path='$targetName')";
    ...
    if($num == 0)
    {
        ...
        if($moved == true){
            $q= "insert  into course_content values ('$course_id','$targetName' , CURRENT_TIMESTAMP, 'Video') ";
            mysqli_query($con, $q);
    ...
    {
        echo "<script>
                alert('Please upload videos with different names!!!');
            </script>";
    }
}
...
if(isset($_FILES['document_upload']))
{
    ...
    $fileTmp=$_FILES['document_upload']['tmp_name'];
    $ModifiedDocumentName = preg_replace('/\s+/', '_', $fileName);
    $targetName="content/".$course_id."_".$ModifiedDocumentName;
    $q = "select * from course_content where (course_id='$course_id' AND file_type='Document' AND file_path='$targetName')";
    $result = mysqli_query($con, $q);
    $num = mysqli_num_rows($result);
    if($num == 0)
    {
        ...
        $moved = move_uploaded_file($fileTmp,$targetName);
        if($moved == true){
            $q= "insert  into course_content values ('$course_id','$targetName' , CURRENT_TIMESTAMP, 'Document') ";
            mysqli_query($con, $q);
            echo "<script>
    ...
    {
        echo "<script>
                        alert('Please upload documents with different names!!!');
            </script>";
    }
}
...
<body>
            ...
            <div class="container-fluid">
	           ...
	           <div class="row bg-title">
                ...
                <div class="row">
			         <div class="col-md-12">
                        <div class="white-box" style="padding:20px">
                            <br><button class="linkButton" style="font-size: 18px" onclick="video_upload()"><i class="fa fa-upload fa-fw" aria-hidden="true"></i>Upload A New Video</button>
                            <div style="display: none;" id="upload_video_div">
                                <form action = "faculty_content.php" method="post" enctype="multipart/form-data">
                                    <br>
                                    <input type="file" id="video_upload" name="video_upload" accept=".mp4,.mov,.flv,.avi,">
                                    <br>
                                    <button type="submit" class="btn btn-info" name="video_submit">Upload</button>
                                </form>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="white-box" style="padding:20px">
                            ...
                            <?php
                                ...
                                while($rows = mysqli_fetch_array($result))
                                {
                                    ...
                                    echo "<div style='padding-left:30px;' class='video_link'>";
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="white-box" style="padding:20px">
                            <br><button class="linkButton" style="font-size: 18px" onclick="document_upload()"><i class="fa fa-upload fa-fw" aria-hidden="true"></i>Upload A New Document</button>
    ...
    <script>
        function video_upload(){
            document.getElementById("upload_video_div").style.display="block";
            document.getElementById("upload_video_div").focus();
        }
        function document_upload(){
            document.getElementById("upload_doc_div").style.display="block";
            document.getElementById("upload_doc_div").focus();
        }
    </script>
</body>
